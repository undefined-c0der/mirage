...
// impl for [16, 64] and any BATCH_SIZE < 16 [x, 64]
// OUTPUT_SIZE = 16, 32, 64, 128, 256, 1600, REDUCTION_SIZE = multiple of 64
...
__device__ __forceinline__ void norm_linear_task_impl(void const *input_ptr,
  ...
  constexpr int CHUNKS_PER_ROW_B = OUTPUT_ATOM_SIZE / CHUNK_SIZE;
  ...
  *((__uint128_t *)smem) = 0ul;
  ...
  T *reduction_output =
      (T *)(smem + 128 + sizeof(T) * BATCH_SIZE * TILE_SIZE +
            ...
            sizeof(T) * TILE_SIZE * OUTPUT_ATOM_SIZE +
            sizeof(T) * BATCH_SIZE * TILE_SIZE +
            sizeof(T) * BATCH_SIZE * OUTPUT_ATOM_SIZE * NUM_WARPS_K +
            sizeof(T) * BATCH_SIZE *
  ...
  using WeightSmem =
      smem_row<T, 3, 3, 3, TILE_SIZE, OUTPUT_ATOM_SIZE, OUTPUT_ATOM_SIZE>;
  using OutputSmem =
...
#pragma unroll
      ...
      for (uint32_t m = 0; m < NUM_ITERS_M; m++) {
        ...
        for (uint32_t n = 0; n < NUM_ITERS_N; n++) {
          int n_col = (n << 6) + (warp_col << 4) + ((lane_idx >> 4) << 3);
#pragma unroll
